[[SAR_WAREHSE.CURRENT_YEAR.AVAL]]
rem --- Verify calendar exists for entered SA fiscal year
	year$=callpoint!.getUserInput()
	if cvs(year$,2)<>"" and year$<>callpoint!.getColumnData("SAR_WAREHSE.CURRENT_YEAR") then
...
[[SAR_WAREHSE.CURRENT_PER.AVAL]]
rem --- Verify haven't exceeded calendar total periods for entered SA fiscal year
	period$=callpoint!.getUserInput()
	if cvs(period$,2)<>"" and period$<>callpoint!.getColumnData("SAR_WAREHSE.CURRENT_PER") then
		period=num(period$)
		...
		if period<1 or period>total_pers then
			...
			msg_tokens$[2]=callpoint!.getColumnData("SAR_WAREHSE.CURRENT_YEAR")
			gosub disp_message
			callpoint!.setStatus("ABORT")
			break
		endif
	endif
[[SAR_WAREHSE.ITEM_ID.AINV]]
...
rem --- create list for available levels
	...
	ldat_list$=pad(Translate!.getTranslation("AON_WAREHOUSE"),20)+"~"+"W;"
	if pos(sas01a.whse_lev$="PI") ldat_list$=ldat_list$+pad(Translate!.getTranslation("AON_PRODUCT"),20)+"~"+"P;"
	if pos(sas01a.whse_lev$="I") ldat_list$=ldat_list$+pad(Translate!.getTranslation("AON_ITEM"),20)+"~"+"I;"

	callpoint!.setTableColumnAttribute("SAR_WAREHSE.SA_LEVEL","LDAT",ldat_list$)
[[SAR_WAREHSE.ASVA]]
rem --- Check selected level against allowable level
	allow=pos(user_tpl.high_level$=user_tpl.sa_levels$)
	if pos(callpoint!.getColumnData("SAR_WAREHSE.SA_LEVEL")=user_tpl.sa_levels$)>allow or
:	   pos(callpoint!.getColumnData("SAR_WAREHSE.SA_LEVEL")=user_tpl.sa_levels$)=0
		msg_id$="INVALID_SA_LEVEL"
		gosub disp_message
		callpoint!.setStatus("ABORT")
	endif
[[SAR_WAREHSE.TWLVE_PER_REPORT.AVAL]]
...
callpoint!.setColumnData("SAR_WAREHSE.SA_LEVEL",sas_params.whse_lev$)
callpoint!.setStatus("REFRESH")
dim user_tpl$:"sa_levels:c(3),high_level:c(1)"
user_tpl.sa_levels$="WPI"
user_tpl.high_level$=sas_params.whse_lev$

rem --- Set maximum number of periods allowed for this fiscal year
	gls_calendar_dev=fnget_dev("GLS_CALENDAR")
	dim gls_calendar$:fnget_tpl$("GLS_CALENDAR")
	current_year$=callpoint!.getColumnData("SAR_WAREHSE.CURRENT_YEAR")
