...
rem --- Verify haven't exceeded calendar total periods for current GL fiscal year
	...
	if cvs(period$,2)<>"" and period$<>callpoint!.getColumnData("GLR_FINANCIALS.PERIOD") then
		...
		if period<1 or period>total_pers then
			...
			msg_tokens$[2]=callpoint!.getColumnData("GLR_FINANCIALS.YEAR")
			gosub disp_message
			callpoint!.setStatus("ABORT")
			break
		endif
	endif
[[GLR_FINANCIALS.YEAR.AVAL]]
rem --- Verify calendar exists for entered GL fiscal year
	year$=callpoint!.getUserInput()
	if cvs(year$,2)<>"" and year$<>callpoint!.getColumnData("GLR_FINANCIALS.YEAR") then
...
if recs_written=0
	...
	open_tables$[1]="GLE_FINANCIALRPT",open_opts$[1]="C"
	gosub open_tables
endif
[[GLR_FINANCIALS.ARAR]]
