...
rem --- Has customer credit been exceeded?
	...
	prev_ext_price = num(callpoint!.getColumnUndoData("OPE_ORDDET.EXT_PRICE"))
...
rem --- Set price and discount
	...
	if std_price then
		callpoint!.setColumnData("OPE_ORDDET.DISC_PERCENT", str(100 - unit_price * 100 / std_price))
	else
		if disc_per <> 100 then
			callpoint!.setColumnData("OPE_ORDDET.STD_LIST_PRC", str(unit_price * 100 / (100 - disc_per)) )
		endif
	endif
...
:		ext_price <> 0
:	then
		callpoint!.setColumnData("OPE_ORDDET.UNIT_PRICE", str(ext_price))
		callpoint!.setColumnData("OPE_ORDDET.EXT_PRICE", "0")
