...
rem --- Verify haven't exceeded calendar total periods for current GL fiscal year
	...
	if cvs(period$,2)<>"" and period$<>callpoint!.getColumnData("GLR_13PERIOD.PERIOD") then
		...
		if period<1 or period>total_pers then
			...
			msg_tokens$[2]=callpoint!.getColumnData("GLR_13PERIOD.YEAR")
			gosub disp_message
			callpoint!.setStatus("ABORT")
			break
		endif
	endif
[[GLR_13PERIOD.YEAR.AVAL]]
rem --- Verify calendar exists for entered GL fiscal year
	year$=callpoint!.getUserInput()
	if cvs(year$,2)<>"" and year$<>callpoint!.getColumnData("GLR_13PERIOD.YEAR") then
...
rem --- Verify haven't exceeded calendar total periods for current GL fiscal year
	period$=callpoint!.getColumnData("GLR_13PERIOD.PERIOD")
