[[GLR_BALANCE.PICK_GL_PER.AVAL]]
rem --- Verify haven't exceeded calendar total periods for current GL fiscal year
	period$=callpoint!.getUserInput()
	if cvs(period$,2)<>"" and period$<>callpoint!.getColumnData("GLR_BALANCE.PICK_GL_PER") then
		period=num(period$)
		...
		if period<1 or period>total_pers then
			...
			msg_tokens$[2]=callpoint!.getColumnData("GLR_BALANCE.PICK_YEAR")
			gosub disp_message
			callpoint!.setStatus("ABORT")
			break
		endif
	endif
[[GLR_BALANCE.PICK_YEAR.AVAL]]
rem --- Verify calendar exists for entered GL fiscal year
	year$=callpoint!.getUserInput()
	if cvs(year$,2)<>"" and year$<>callpoint!.getColumnData("GLR_BALANCE.PICK_YEAR") then
...
rem --- Verify haven't exceeded calendar total periods for current GL fiscal year
	period$=callpoint!.getColumnData("GLR_BALANCE.PICK_GL_PER")
...
rem ==========================================================================
	...
	if alignCalendar!.canAlignCalendar(pick_year$) then
		...
		callpoint!.setColumnEnabled("GLR_BALANCE.ALIGN_PERIODS",1)
	else
		rem --- canNOT align calendar
		callpoint!.setColumnEnabled("GLR_BALANCE.ALIGN_PERIODS",0)
	endif
	callpoint!.setColumnData("GLR_BALANCE.ALIGN_PERIODS","N",1)
...
callpoint!.setColumnData("GLR_BALANCE.PICK_GL_PER",gls01a.current_per$)
callpoint!.setColumnData("GLR_BALANCE.PICK_YEAR",gls01a.current_year$)
callpoint!.setStatus("REFRESH")

rem --- Set maximum number of periods allowed for this fiscal year
	gls_calendar_dev=fnget_dev("GLS_CALENDAR")
	dim gls_calendar$:fnget_tpl$("GLS_CALENDAR")
	current_year$=callpoint!.getColumnData("GLR_BALANCE.PICK_YEAR")
