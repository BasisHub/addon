[[SAR_CUSTSHIP.CURRENT_YEAR.AVAL]]
rem --- Verify calendar exists for entered SA fiscal year
	year$=callpoint!.getUserInput()
	if cvs(year$,2)<>"" and year$<>callpoint!.getColumnData("SAR_CUSTSHIP.CURRENT_YEAR") then
...
[[SAR_CUSTSHIP.CURRENT_PER.AVAL]]
rem --- Verify haven't exceeded calendar total periods for entered SA fiscal year
	period$=callpoint!.getUserInput()
	if cvs(period$,2)<>"" and period$<>callpoint!.getColumnData("SAR_CUSTSHIP.CURRENT_PER") then
		period=num(period$)
		...
		if period<1 or period>total_pers then
			...
			msg_tokens$[2]=callpoint!.getColumnData("SAR_CUSTSHIP.CURRENT_YEAR")
			gosub disp_message
			callpoint!.setStatus("ABORT")
			break
		endif
...
rem --- Check selected level against allowable level
	...
	if pos(callpoint!.getColumnData("SAR_CUSTSHIP.SA_LEVEL")=user_tpl.sa_levels$)>allow or
:	   pos(callpoint!.getColumnData("SAR_CUSTSHIP.SA_LEVEL")=user_tpl.sa_levels$)=0
...
[[SAR_CUSTSHIP.TWLVE_PER_REPORT.AVAL]]
...
callpoint!.setColumnData("SAR_CUSTSHIP.SA_LEVEL",sas_params.shipto_lev$)
callpoint!.setStatus("REFRESH")
dim user_tpl$:"sa_levels:c(3),high_level:c(1)"
user_tpl.sa_levels$="CSI"
user_tpl.high_level$=sas_params.shipto_lev$

rem --- Set maximum number of periods allowed for this fiscal year
	gls_calendar_dev=fnget_dev("GLS_CALENDAR")
	dim gls_calendar$:fnget_tpl$("GLS_CALENDAR")
	current_year$=callpoint!.getColumnData("SAR_CUSTSHIP.CURRENT_YEAR")
