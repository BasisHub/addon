[[SAR_ITEM.CURRENT_YEAR.AVAL]]
rem --- Verify calendar exists for entered SA fiscal year
	year$=callpoint!.getUserInput()
	if cvs(year$,2)<>"" and year$<>callpoint!.getColumnData("SAR_ITEM.CURRENT_YEAR") then
...
rem --- Verify haven't exceeded calendar total periods for entered SA fiscal year
	...
	if cvs(period$,2)<>"" and period$<>callpoint!.getColumnData("SAR_ITEM.CURRENT_PER") then
		...
		if period<1 or period>total_pers then
			...
			msg_tokens$[2]=callpoint!.getColumnData("SAR_ITEM.CURRENT_YEAR")
			gosub disp_message
			callpoint!.setStatus("ABORT")
			break
		endif
...
rem --- Check selected level against allowable level
	...
	if pos(callpoint!.getColumnData("SAR_ITEM.SA_LEVEL")=user_tpl.sa_levels$)>allow or
:	   pos(callpoint!.getColumnData("SAR_ITEM.SA_LEVEL")=user_tpl.sa_levels$)=0
		msg_id$="INVALID_SA_LEVEL"
		gosub disp_message
		callpoint!.setStatus("ABORT")
	endif
[[SAR_ITEM.TWLVE_PER_REPORT.AVAL]]
...
callpoint!.setColumnData("SAR_ITEM.SA_LEVEL",sas_params.product_lev$)
callpoint!.setStatus("REFRESH")
dim user_tpl$:"sa_levels:c(2),high_level:c(1)"
user_tpl.sa_levels$="PI"
user_tpl.high_level$=sas_params.product_lev$

rem --- Set maximum number of periods allowed for this fiscal year
	gls_calendar_dev=fnget_dev("GLS_CALENDAR")
	dim gls_calendar$:fnget_tpl$("GLS_CALENDAR")
	current_year$=callpoint!.getColumnData("SAR_ITEM.CURRENT_YEAR")
