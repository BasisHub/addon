[[SAR_PRODITMCST.CURRENT_YEAR.AVAL]]
rem --- Verify calendar exists for entered SA fiscal year
	year$=callpoint!.getUserInput()
	if cvs(year$,2)<>"" and year$<>callpoint!.getColumnData("SAR_PRODITMCST.CURRENT_YEAR") then
...
[[SAR_PRODITMCST.CURRENT_PER.AVAL]]
rem --- Verify haven't exceeded calendar total periods for entered SA fiscal year
	period$=callpoint!.getUserInput()
	if cvs(period$,2)<>"" and period$<>callpoint!.getColumnData("SAR_PRODITMCST.CURRENT_PER") then
		period=num(period$)
		...
		if period<1 or period>total_pers then
			...
			msg_tokens$[2]=callpoint!.getColumnData("SAR_PRODITMCST.CURRENT_YEAR")
			gosub disp_message
			callpoint!.setStatus("ABORT")
			break
		endif
...
rem --- Set maximum number of periods allowed for this fiscal year
	gls_calendar_dev=fnget_dev("GLS_CALENDAR")
	dim gls_calendar$:fnget_tpl$("GLS_CALENDAR")
	current_year$=callpoint!.getColumnData("SAR_PRODITMCST.CURRENT_YEAR")
