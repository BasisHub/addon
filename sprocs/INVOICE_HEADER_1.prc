...
data!.setFieldValue("INTERNATIONAL", str(123))
...
if itemListVat01$<>"" then
    sql$ = "SELECT UNIT_PRICE,QTY_SHIPPED FROM OPE_ORDDET WHERE ITEM_ID IN ("+ itemListVat01$ +") AND FIRM_ID='" + firm_id$ + "' AND CUSTOMER_ID='" + customer_id$ + "' AND ORDER_NO='" + order_no$ + "'"
    sqlRs! = BBJAPI().createSQLRecordSet(url$,mode$,sql$)
    sqlRd! = sqlRs!.getCurrentRecordData()
    sql$ = "SELECT QTY_CHARGED FROM STO_ORDDET WHERE ITEM_ID IN ("+ itemListVat01$ +") AND FIRM_ID='" + firm_id$ + "' AND ORDER_NO='" + order_no$ + "'"
    sqlRs2! = BBJAPI().createSQLRecordSet(url$,mode$,sql$)
    sqlRd2! = sqlRs2!.getCurrentRecordData()
    if !(sqlRs!.isEmpty()) then
        while 1
            sqlRd! = sqlRs!.getCurrentRecordData()
            total_vat_01 = total_vat_01 + (vat_01*num(sqlRd!.getFieldValue("UNIT_PRICE"))*(num(sqlRd!.getFieldValue("QTY_SHIPPED"))-num(sqlRd2!.getFieldValue("QTY_CHARGED"))))/100
            sqlRs!.next(err=*break)
